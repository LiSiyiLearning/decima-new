...
def invoke_model(actor_agent, obs, exp):
    ...
    # for storing type index
    type_act_vec = np.zeros(type_act_probs.shape)
    type_act_vec[0, job_idx, type_act[0, job_idx]] = 1
    # store experience
...
def main():
    ...
    for ep in range(1, args.num_ep):
        ...
        all_cum_reward = []
        avg_per_step_reward = avg_reward_calculator.get_avg_per_step_reward()  # 这里是每一个step的reward
        for i in range(args.num_agents):
        ...
        # decrease reset probability 貌似是这里
        ...
        # print('whh', actor_agent.batchsize)
        # print('wht', actor_agent.test)
    sess.close()
