...
def generate_tianchi_jobs(np_random, timeline, wall_time):
    ...
    for i in range(args.job_num):
        ...
        data_path = os.path.join(cur_path, "data_d")
      
        adj_mat = np.load('{}/adj_mat_{}.npy'.format(data_path, i + 1), allow_pickle=True)
        nodes_info = np.load('{}/job_info_{}.npy'.format(data_path, i + 1), allow_pickle=True).item()
        ...
        for n in range(num_nodes):
            ...
            # generate a node
            node = Node(n, tasks, duration, wall_time, np_random, node_info["cpu"], node_info["mem"])
            nodes.append(node)
